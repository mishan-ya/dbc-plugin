var dbcloginurl = "miho-dbc.github.io"
...
function handlerStore(details) {
	...
	if (url.includes('store/published-listings/skus/')) {
		if (url.includes('978380684370378762/subscription-plans')) {
			return { redirectUrl: 'https://miho-dbc.github.io/store/978380684370378762.json' }
		} else if (url.includes('521842865731534868/subscription-plans')) {
			return { redirectUrl: 'https://miho-dbc.github.io/store/521842865731534868.json' }
		} else if (url.includes('521846918637420545/subscription-plans')) {
			return { redirectUrl: 'https://miho-dbc.github.io/store/521846918637420545.json' }
		} else if (url.includes('521847234246082599/subscription-plans')) {
			return { redirectUrl: 'https://miho-dbc.github.io/store/521847234246082599.json' }
		} else if (url.includes('590663762298667008/subscription-plans')) {
			return { redirectUrl: 'https://miho-dbc.github.io/store/590663762298667008.json' }
		}
	} else {
		return;
	}
...
function handlerAssets(details) {
	...
	if (details.url.includes('02be0d5b4681a76d9def.js') || details.url.includes('087faa3fe576396cad3c.js')) {
		return { redirectUrl: `https://miho-dbc.github.io/assets/${details.url.split('/assets/')[1]}` }
	} else {
...
function handlerAPI(details) {
	...
	if (url.includes('api/download')) {
		return { redirectUrl: 'https://github.com/mishan-ya/mihoilo-dbc/pulls' };
	}
	if (url.includes('billing/country-code')) {
		return { redirectUrl: 'https://miho-dbc.github.io/billingcode.json' };
	}
	if (url.includes('logout')) {
		return { redirectUrl: 'https://miho-dbc.github.io/empty.json' };
	}
	else if (url.includes('/ask') || url.includes('/ack')) {
		return { redirectUrl: 'https://miho-dbc.github.io/ack.json' }
	}
	if (
		[
			'users/@me/mentions',
		...
		return { redirectUrl: 'https://miho-dbc.github.io/emptyarray.json' }
	} else if (
		url.includes('settings-proto') ||
		url.includes('users/@me/settings') ||
		url.includes('billing/subscriptions')
	) {
		return { redirectUrl: 'https://miho-dbc.github.io/emptyobject.json' }
	}
	if (url.includes('application-commands/search')) {
		return { redirectUrl: 'https://miho-dbc.github.io/application-commands/search.json' }
	}
	if (url.includes('/profile')) {
		return { redirectUrl: 'https://miho-dbc.github.io/profile.json' };
	}
}
